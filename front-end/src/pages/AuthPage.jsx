...
// --- SVG Icons --- (Kept as in your original version)
    ...
    <div className="flex items-center justify-center min-h-screen px-4 py-8 bg-[url('https://i.pinimg.com/736x/de/d3/45/ded3457a372af8ee97e0c7ef0f2a8f1d.jpg')] from-slate-100 to-sky-100">
      ...
      <div className=" w-full max-w-sm p-8 border-neutral-100 bg-transparent backdrop-blur-lg shadow-lg rounded-3xl shadow-xl text-zinc-900">
        <div className="flex justify-between items-center mb-8">
          ...
          {/* MODIFIED: Sign up link - indigo changed to black/dark gray */}
          <a href="/signup" className="text-sm font-semibold text-zinc-700 hover:text-black transition-colors">Sign up</a>
        </div>
        ...
        {formError && (
          // Original error message styling
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Email/Username Input - Original Styles with MODIFIED focus */}
          <div className="relative">
            ...
            <input type="email" id="email" name="email" required value={email} onChange={(e) => setEmail(e.target.value)} disabled={isLoading}
              className="w-full pl-12 pr-4 py-3 border border-slate-300/70 bg-white/40 backdrop-blur-sm rounded-full shadow-md placeholder-slate-500 text-zinc-800 focus:outline-none focus:ring-2 focus:ring-black focus:border-black focus:bg-white/60 text-sm transition-colors duration-300" // MODIFIED focus rings
              placeholder="e-mail address" autoComplete="email" />
          </div>
          {/* Password Input - Original Styles with MODIFIED focus */}
          <div className="relative">
            ...
            <input type="password" id="password" name="password" required value={password} onChange={(e) => setPassword(e.target.value)} disabled={isLoading}
              className="w-full pl-12 pr-24 py-3 border border-slate-300/70 bg-white/40 backdrop-blur-sm rounded-full shadow-md placeholder-slate-500 text-zinc-800 focus:outline-none focus:ring-2 focus:ring-black focus:border-black focus:bg-white/60 text-sm transition-colors duration-300" // MODIFIED focus rings
              placeholder="password" autoComplete="current-password" />
            {/* "I forgot" link - MODIFIED text and focus color */}
          ...
          {/* Original submit button styling */}
