...
export default function DynamicActionBar() {
  ...
  // const duration = 0.75; // These seem unused, can be removed if not needed elsewhere
  // const duration2 = 0.3;
  ...
  const collapseDetails = (resetIdx = true) => {
    ...
    containerRef.current.style.borderRadius = `${getResponsiveValue('16px', '16px')}`; // Use 'px' for consistency or ensure Tailwind handles it
  ...
  const handleLoginRegClick = () => {
   ...
   const handleProfilePageClick = () => { // This is the correctly spelled function
    if (expandedIndex !== -1) {
      collapseDetails();
    }
    if (currentUser && currentUser._id) {
      navigate(`/profile/${currentUser._id}`); // Correctly uses template literal
    } else {
      navigate('/login');
      console.warn("DynamicActionBar: Attempted to navigate to profile without a current user ID.");
    }
  };
    ...
    <>
      ...
      <div
        ...
        className={`
          ...
          left-0 right-0 mx-4 h-14 md:mx-0 md:left-1/2 md:right-auto md:h-12 md:w-[410px] md:-translate-x-1/2
          transition-[height] duration-100
        `} // Removed mx-22 as it was likely a typo or overridden by md:mx-0
         style={{
            ...
            {/* --- Details Content --- */}
             {index === 0 && (
                 <>
                   ...
                   <div onClick={handleSignOutClick} className="w-[95%] flex items-center gap-3 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 rounded-2xl py-3 px-3 duration-300">

                     <div className="w-full flex flex-col items-start"><p className="font-bold">Sign Out</p><p className="opacity-80">Change Your account</p></div>
                    </div>
...
{/* */}
             ...
             {index === 2 && isAuthenticated && currentUser &&( // Now currentUser and isAuthenticated are defined
                 <>
                    <div className="w-full group">
                      <div onClick={handleProfilePageClick}  className="flex items-center justify-between w-full gap-3 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 rounded-2xl py-2 px-3 duration-300 mx-auto">
                        ...
                        <span>Jun, 24</span> {/* Consider making this dynamic if it's a date */}
                      </div>
                    </div>
      ...
      <div
        ...
        className="
          ...
          z-50 border border-gray-200/50 dark:border-gray-700/50 rounded-2xl bg-neutral-100/80 backdrop-blur-md
          left-0 right-0 mx-4 p-1 md:mx-0 md:p-0 md:gap-2 md:left-1/2 md:right-auto md:w-[410px] md:-translate-x-1/2
        " // Removed mx-20 as it was conflicting with mx-4 and md:mx-0
      ...
      >
        ...
        >
          ...
          <span className="font-bold hidden md:inline">USER</span> {/* Changed from Share to USER to match content */}
        </button>
      </div>
    </>
  );
